...
async function buildHeader() {
    ...
    const logoImg = settings.logoImageUrl ? `<img src="${optimizeImage(logoUrl, 200)}" alt="Logo" class="header-logo-img">` : '';
...
async function buildFooter() {
    ...
    const companyName = settings.logoText ? settings.logoText : '';
    ...
    // Updated Layout with 3 Columns
    footerElement.innerHTML = `
        <div class="footer-container-new">
            <div class="footer-column" id="footer-col-about">
                <span class="footer-column-title" data-target="footer-content-1">
                    ABOUT <svg class="footer-accordion-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><path fill-rule="evenodd" d="M8 11.293l-4.646-4.647a.5.5 0 0 1 .708-.708L8 9.879l4.939-4.939a.5.5 0 0 1 .708.708L8 11.293z"></path></svg>
                </span>
                <ul class="footer-links-list" id="footer-content-1">
            ...
            <div class="footer-column" id="footer-col-links">
                <span class="footer-column-title" data-target="footer-content-3">
                    QUICK LINKS <svg class="footer-accordion-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16"><path fill-rule="evenodd" d="M8 11.293l-4.646-4.647a.5.5 0 0 1 .708-.708L8 9.879l4.939-4.939a.5.5 0 0 1 .708.708L8 11.293z"></path></svg>
                </span>
            ...
            <div class="footer-column social-column">
                <span class="footer-column-title">FOLLOW US</span>
                <div class="footer-social-wrapper">${socialLinksHTML}</div>
            </div>

            <div class="footer-bottom-new">${copyRightHTML}${poweredByHTML}</div>
        </div>
    `;
...
function setupFooterAccordion() {
    ...
    titles.forEach(title => {
        title.addEventListener('click', () => {
            // Check if we are on mobile (screen width < 769px)
            if (window.innerWidth > 768) return;
            
            const targetId = title.dataset.target;
            if (!targetId) return; // For titles without targets (like Follow Us)
...
function buildUserMenuHTML(settings) {
    ...
    <div class="user-menu-overlay" id="user-menu-overlay">
        <div class="user-menu-content">
            <div class="user-menu-header"><h3>My Account</h3><button class="user-menu-close-btn" id="user-menu-close-btn"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg></button></div>
...
export async function loadSiteSettings() {
    ...
    } catch (error) { console.error("Error during site initialization: ", error); } 
    finally {
        if (preloader) {
