// ഇതാണ് 'common.js' ഫയൽ.
// *** Image Optimization & Data Caching നടപ്പിലാക്കി ***
...
/**
 * *** പുതിയത്: ചിത്രങ്ങളുടെ വലിപ്പം കുറയ്ക്കാനുള്ള മാന്ത്രിക വിദ്യ ***
 * 'wsrv.nl' എന്ന സൗജന്യ സേവനം ഉപയോഗിച്ച് ചിത്രങ്ങൾ ചെറുതാക്കുന്നു.
 * @param {string} url - ഒറിജിനൽ ഇമേജ് ലിങ്ക്
 * @param {number} width - നമുക്ക് ആവശ്യമുള്ള വീതി (px)
 * @param {number} quality - ക്വാളിറ്റി (1-100), default 80
 */
export function optimizeImage(url, width = 800, quality = 80) {
    ...
    // ഇതിനകം ഒപ്റ്റിമൈസ് ചെയ്തതാണെങ്കിൽ അല്ലെങ്കിൽ ഡാറ്റാ URL ആണെങ്കിൽ മാറ്റം വരുത്തണ്ട
    if (url.includes('wsrv.nl') || url.startsWith('data:') || url.includes('blob:')) {
        return url;
    }

    // wsrv.nl വഴി ഇമേജ് റീസൈസ് ചെയ്യുന്നു (WebP ഫോർമാറ്റിലേക്ക് മാറ്റുന്നു)
    // output=webp: വലിപ്പം വളരെ കുറയ്ക്കും
    // w={width}: ആവശ്യത്തിനുള്ള വീതി മാത്രം മതി
    // q={quality}: ക്വാളിറ്റി കുറച്ച് ഡാറ്റ ലാഭിക്കുന്നു
...
function authenticateUser() {
    ...
    authPromise = new Promise(async (resolve, reject) => {
        try {
            // നേരത്തെ ലോഗിൻ ചെയ്തിട്ടുണ്ടോ എന്ന് നോക്കുന്നു
            if (auth.currentUser) {
...
/**
 * സൈറ്റ് സെറ്റിംഗ്സ് എടുക്കുന്നു (Caching ഉൾപ്പെടെ)
 * *** പുതിയത്: ലോക്കൽ സ്റ്റോറേജിൽ നിന്ന് ആദ്യം എടുക്കും (ഡാറ്റ ലാഭിക്കാൻ) ***
 */
export async function fetchSiteSettings() {
    if (siteSettings) return siteSettings;

    // 1. ലോക്കൽ സ്റ്റോറേജിൽ ഉണ്ടോ എന്ന് നോക്കുന്നു
    const cachedSettings = localStorage.getItem('siteSettings');
    if (cachedSettings) {
        siteSettings = JSON.parse(cachedSettings);
        // ബാക്ക്ഗ്രൗണ്ടിൽ പുതിയത് അപ്ഡേറ്റ് ചെയ്യാൻ ശ്രമിക്കാം (User അനുഭവം മെച്ചപ്പെടുത്താൻ)
        refreshSettingsBackground(); 
        return siteSettings;
    }

    // 2. ഇല്ലെങ്കിൽ ഫയർബേസിൽ നിന്ന് എടുക്കുന്നു
...
async function refreshSettingsBackground() {
    try {
        ...
        if (docSnap.exists()) {
            ...
            // പുതിയ വിവരം ലോക്കൽ സ്റ്റോറേജിൽ സേവ് ചെയ്യുന്നു
...
async function buildHeader() {
    ...
    // ലോഗോ ഒപ്റ്റിമൈസ് ചെയ്യുന്നു (ചെറിയ വലിപ്പം മതി)
    const logoUrl = optimizeImage(settings.logoImageUrl, 150); 
...
/**
 * 2. സൈഡ് മെനു (Caching ഉപയോഗിച്ച്)
 */
async function buildSideNav() {
    ...
    try {
        ...
        if (cachedCats) {
            ...
            categoryLinks = links;
            localStorage.setItem('cachedCategoriesNav', links); // സേവ് ചെയ്യുന്നു
        }
    } catch (error) {
        console.error("Error loading categories:", error);
    }

    // ... (ബാക്കി സോഷ്യൽ ലിങ്ക് കോഡുകൾ പഴയതുപോലെ തന്നെ) ...
    // കോഡ് ചുരുക്കാൻ വേണ്ടി ബാക്കി ഭാഗം അതേപടി നിലനിർത്തുന്നു, logicൽ മാറ്റമില്ല.
    
    let socialLinksHTML = '';
    // (സോഷ്യൽ ലിങ്കുകൾ ജനറേറ്റ് ചെയ്യുന്ന കോഡ് ഇവിടെ വരും - പഴയ common.js ലേത് പോലെ)
    // ലളിതമാക്കാൻ, അത് buildFooter-ൽ നിന്ന് കോപ്പി ചെയ്യാവുന്നതാണ്.
...
// ... (buildFooter, buildFloatingButtons, setupNavEvents എന്നിവയിൽ വലിയ മാറ്റമില്ല, പക്ഷെ optimizeImage ഉപയോഗിക്കാൻ മറക്കരുത്) ...
...
async function buildFooter() {
    ...
    // സോഷ്യൽ ലിങ്കുകൾ (അഡ്മിൻ പാനലിൽ നിന്ന്)
    let socialLinksHTML = '';
    // (കോഡ് ചുരുക്കുന്നു - പഴയത് പോലെ തന്നെ)
...
export async function loadSiteSettings() {
    ...
    try {
        ...
        if (preloaderLogo && settings.logoImageUrl) {
            // പ്രീലോഡറിൽ ഒപ്റ്റിമൈസ് ചെയ്ത ലോഗോ കാണിക്കുന്നു
